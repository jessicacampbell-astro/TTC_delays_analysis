...
GROUP BY d22.Station, Line, Line_Num
-- order by total delay time
ORDER BY (ROUND((SUM(d23.Min_Delay)-SUM(d22.Min_Delay))/SUM(d22.Min_Delay))) DESC;
...
SELECT
  ...
  -- number of delays
  COUNT(*) AS Num_Delays,
  -- classification of number of delays
  CASE
  	WHEN COUNT(*) BETWEEN 0 AND 50 THEN 'Low number of delays.'
  	WHEN COUNT(*) BETWEEN 51 AND 100 THEN 'Medium number of delays.'
  	WHEN COUNT(*) > 100 THEN 'High number of delays.'
  -- give classification column an alias
  END AS Num_Class,
  -- total delay in minutes
  SUM(Min_Delay) AS Sum_Delay,
  -- classification of total delay time
  CASE
  	WHEN SUM(Min_Delay) BETWEEN 0 AND 500 THEN 'Low total delay time.'
  	WHEN SUM(Min_Delay) BETWEEN 501 AND 1000 THEN 'Medium total delay time.'
  	WHEN SUM(Min_Delay) > 1000 THEN 'High total delay time.'
  -- give classification column an alias
  END AS Sum_Class,
  -- average delay in minutes
  FORMAT(ROUND(AVG(Min_Delay),2),2) AS Avg_Delay,
  -- minimum delay in minutes
  MIN(Min_Delay) AS Min_Delay,
  -- maximum delay in minutes
  MAX(Min_Delay) AS Max_Delay,
  -- standard deviation delay in minutes
  -- round and format to 2 decimal places
  FORMAT(ROUND(STD(Min_Delay),2),2) AS Std_Delay,
  -- correlated sub-query for change in total delay time
  (SELECT
  	-- ratio of delay between 2023 and 2022
  	CONCAT(ROUND((SUM(d23.Min_Delay)-SUM(d22.Min_Delay))/SUM(d22.Min_Delay)*100,0),'%')
    AS Delay_Change
    FROM delays_2022 d22
	  -- join 2022 and 2023 tables by station
	  JOIN delays_2023 d23
	  -- join between 2022 and 2023 tables
	  -- and join on station for outer query
	  ON (d22.Station = d23.Station) AND (d22.Station = d22_outer.Station)
	  -- restrict to Jan-Aug
    WHERE (d22.Date BETWEEN '2022/01/01' AND '2022/10/31')
  ) AS Delay_Change
FROM delays_2022 d22_outer
-- restrict to non-zero delay times
WHERE Min_Delay>0
-- group by station, line number, then line name
GROUP BY Station, Line_Num, Line
-- ORDER BY station then line number
ORDER BY Station, Line_Num;
